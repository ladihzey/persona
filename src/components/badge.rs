...
pub fn Badge(cx: Scope, children: Children, variant: BadgeVariant) -> impl IntoView {
    match variant {
        ...
        Yellow => view! { cx,
            <span class="inline-flex items-center rounded-md bg-yellow-800/40 px-1 md:px-2 py-1 text-xs font-medium text-yellow-600 ring-1 ring-inset ring-yellow-500/20">{children(cx)}
            </span>
        },
        Green => view! { cx,
